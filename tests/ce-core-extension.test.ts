...
describe("commands/pedstack: session-traversal helpers", () => {
	...
	describe("cmdPedNext", () => {
		test("with clean state resolves and sends skill command", async () => {
			...
			// Should have sent ONE message: /skill:01-brainstorm (fallback when no pending path)
			expect(sentMessages.length).toBe(1);
			expect(sentMessages[0].content).toBe("/skill:01-brainstorm");
		});
	...
	describe("THINKING_LEVEL_MAP", () => {
		test("maps all 9 entries correctly", () => {
			const expected = {
				off: "off",
				minimal: "minimal",
				low: "low",
			...
			// Import the internal map by accessing the module
			// We test it through switchThinkingLevel indirectly,
			// but we can also verify the exported isValidStageKey covers valid keys
			expect(isValidStageKey("01-brainstorm")).toBe(true);
			expect(isValidStageKey("06-docsync")).toBe(true);
			expect(isValidStageKey("99-invalid")).toBe(false);
...
describe("ce-core extension registers commands", () => {
	test("registers ped-start and ped-next commands", () => {
