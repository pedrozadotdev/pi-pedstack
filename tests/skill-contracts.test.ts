...
describe("skill package contracts", () => {
  ...
  test("00-next provides both next-step recommendation and full status report", () => {
    const content = readFileSync(path.join(repoRoot, "skills", "00-next", "SKILL.md"), "utf8")
  ...
  test("00-next uses workflow_state to recommend the next skill", () => {
    const content = readFileSync(path.join(repoRoot, "skills", "00-next", "SKILL.md"), "utf8")
    const recommendationLogic = readFileSync(
      path.join(repoRoot, "skills", "00-next", "references", "recommendation-logic.md"),
      "utf8",
    )
    ...
    expect(recommendationLogic).toContain("debug")
    expect(recommendationLogic).toContain("learn")
    expect(recommendationLogic).toContain("docsync")
  })
