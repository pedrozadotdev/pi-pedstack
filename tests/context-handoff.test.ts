...
describe("context_handoff", () => {
	...
	test("save writes latest handoff markdown and dated handoff file", async () => {
		...
		const savedText = readFileSync(
			path.join(repoRoot, result.latestPath!),
			"utf8",
		);
	...
	test("status recommends new session only for heavy/critical cross-phase", async () => {
		...
		const samePhaseHeavy = await tool.execute({
			operation: "status",
			repoRoot,
		});
		...
		const watchCrossPhase = await tool.execute({
			operation: "status",
			repoRoot,
		});
	...
	test("load returns latest handoff metadata and markdown", async () => {
		...
		expect(result.handoffMarkdown).toContain("/ped-next");
	});
	...
	test("save persists all five new structured fields in context-state.json", async () => {
		...
		const result = await tool.execute({
			...
			currentTruth: [
				"The API returns 200 for valid requests",
		...
		expect(result.openDecisions).toEqual([
			"Choose between REST and GraphQL for v2",
		]);
		expect(result.recentlyAccessedFiles).toEqual([
			"src/a.ts",
			"src/b.ts",
			"src/c.ts",
		]);
		expect(result.compressionRisk).toEqual([
			"Large test output may push context over budget",
		]);
		...
		expect(state.invalidatedAssumptions).toEqual([
			"Legacy format is still supported",
		]);
		expect(state.openDecisions).toEqual([
			"Choose between REST and GraphQL for v2",
		]);
		expect(state.recentlyAccessedFiles).toEqual([
			"src/a.ts",
			"src/b.ts",
			"src/c.ts",
		]);
		expect(state.compressionRisk).toEqual([
			"Large test output may push context over budget",
		]);
	});
	...
	test("load/latest/status return new structured fields", async () => {
		...
		expect(latestResult.recentlyAccessedFiles).toEqual([
			"file1.ts",
			"file2.ts",
		]);
		...
		expect(statusResult.recentlyAccessedFiles).toEqual([
			"file1.ts",
			"file2.ts",
		]);
	...
	test("save/load/latest/status round-trip activeRules", async () => {
		...
		expect(saveResult.activeRules).toEqual([
			"TDD gate: RED→GREEN→REFACTOR",
			"No business logic changes",
		]);
		...
		const latestResult = await tool.execute({ operation: "latest", repoRoot });
		expect(latestResult.activeRules).toEqual([
			"TDD gate: RED→GREEN→REFACTOR",
			"No business logic changes",
		]);
	...
	test("validate returns repo-relative path for explicit absolute repo handoff path", async () => {
		...
		expect(result.path).toBe(
			".context/compound-engineering/handoffs/custom.md",
		);
	});
	...
	test("validate normalizes absolute state handoff path in public result", async () => {
		...
		expect(result.found).toBe(true);
		expect(result.path).toBe(
			".context/compound-engineering/handoffs/legacy.md",
		);
	});
});
